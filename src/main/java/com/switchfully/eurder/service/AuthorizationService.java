...
public class AuthorizationService {
    ...
    public void throwExceptionIfNotAdmin(String userId) throws IllegalAccessException {
        if (!ValidationUtil.isValidUUID(userId) || !isAdmin(UUID.fromString(userId))) {
    ...
    public void throwExceptionIfNotCustomer(String userId) throws IllegalAccessException {
        if (!ValidationUtil.isValidUUID(userId) || !isCustomer(UUID.fromString(userId))) {
    ...
    public void throwExceptionIfNotOwnProfile(String authorizationId, String customerId) throws IllegalAccessException {
        if (ValidationUtil.isNullObject(authorizationId) || !authorizationId.equals(customerId) && !isCustomer(UUID.fromString(customerId))) {
    ...
    public void throwExceptionIfNotMember(String userId) throws IllegalAccessException {
        if (!ValidationUtil.isValidUUID(userId) || !isUser(UUID.fromString(userId))) {
    ...
    private boolean isUser(UUID uuid) {
        return userRepository.getUserById(uuid) != null;
    }


    private boolean isAdmin(UUID uuid) {
        return isUser(uuid) && userRepository.getUserById(uuid).getUserRole() == UserRole.ADMIN;
    }

    private boolean isCustomer(UUID uuid) {
        return isUser(uuid) && userRepository.getUserById(uuid).getUserRole() == UserRole.CUSTOMER;
    }
